#!/usr/bin/env tsx
/**
 * Root-level Deployment Script
 *
 * All-in-one command that:
 * 1. Updates app version in database (web)
 * 2. Stages all changes (web + mobile)
...
// Step 1: Update version in database (web project)
...
try {
  ...
  if (!existsSync(versionScript)) {
    throw new Error(
      "apps/web/scripts/update-app-version.ts not found"
    );
  }
  ...
  // Change to web directory to ensure proper environment variable loading
...
// Step 2: Stage all changes (from root — includes web + mobile)
console.log("\n📝 Step 2/4: Staging changes...");
try {
  execSync("git add .", { stdio: "inherit" });
  console.log("✅ Changes staged (web + mobile)");
} catch {
...
console.log(
  "   Changes have been pushed to remote (includes web + mobile)\n"
);
